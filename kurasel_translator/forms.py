...
class PaymentAuditForm(KuraselTranslatorForm):
    """Kuraselの承認された支払管理データを読み込むためのForm"""

    day = forms.ChoiceField(
        label="支払日", widget=forms.Select(attrs={"class": "select-css"}), choices=DATE_PAYMENT
    )


class BalanceSheetTranslateForm(KuraselTranslatorForm):
    """貸借対照表入力フォーム
    - Kuraselでは管理会計、修繕会計に同じ「受取利息」費目があるため、
    管理会計では「受取利息」を「雑収入」として記録するため、会計区分フラグを使う。
    （口座は一つなのに...）
...
class ClaimTranslateForm(KuraselTranslatorForm):
    """管理費等請求一覧データ入力フォーム"""
