...
from monthly_report.forms import DeleteByYearMonthAcclass, MonthlyReportIncomeForm
...
class MonthlyReportIncomeListView(MonthlyReportBaseView):
    ...
    def get_context_data(self, **kwargs):
        ...
        qs = get_monthly_report_queryset(tstart, tend, ac_class, "income", is_chonaikai)
...
# UpdateView
# ----------------------------------------------------------------------------
class MonthlyReportIncomeUpdateView(PermissionRequiredMixin, UpdateView):
    """月次報告の収入データの摘要欄更新用UpdateView"""
...
# ----------------------------------------------------------------------------
# 月次収入データの個別削除用 DeleteView
# ----------------------------------------------------------------------------
class DeleteIncomeView(PermissionRequiredMixin, DeleteView):
    """月次報告収入データ削除View
    摘要欄にコメントを追加すると、ククラセからの再読み込みがスキップされるため
    削除処理が必要になる。
    """
...
# ----------------------------------------------------------------------------
# 月次収入データの一括削除用 FormView
# ----------------------------------------------------------------------------
class MonthlyIncomeDeleteByYearMonthView(MonthlyReportBaseView, FormView):
    """指定された年月の収入データを一括削除するFormView"""
